  ...
  open func drawCubicBezier(context: CGContext, dataSet: ILineChartDataSet)
  {
    ...
    if (size - minx >= 2)
    {
      ...
      for j in stride(from: (minx + 1), to: min(size, entryCount), by: 1)
      {
        ...
        cubicPath.addCurve(
          to: CGPoint(
            x: CGFloat(cur.xIndex),
            y: CGFloat(cur.value) * phaseY),
          control1: CGPoint(
            x: CGFloat(prev.xIndex) + prevDx,
            y: (CGFloat(prev.value) + prevDy) * phaseY),
          control2: CGPoint(
            x: CGFloat(cur.xIndex) - curDx,
            y: (CGFloat(cur.value) - curDy) * phaseY),
